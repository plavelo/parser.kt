...
class RegexTest {
    ...
    fun regex() {
        ...
        Assert.assertEquals("1", parser1.parse("1").right().content())
        Assert.assertTrue(parser1.parse("x0").isLeft())
        Assert.assertEquals("1", Parser.regex("[0-9]")("1", 0).right().content())
        ...
        Assert.assertEquals("abc", result.right().content())
    }
}
